      ...
      <main>
          ...
          {listing && !loading && !error && (
            ...
            <div className='flex gap-4'>
                <p className='bg-red-900 w-full max-w-[100px] text-white
                text-center p-1 rounded-md items-center'>
                    {listing.type === 'rent' ? 'For Rent' : 'For Sale'}
                </p>
                {
                    listing.offer && (
                              <p className='bg-green-900 w-full max-w-[100px] text-white
                text-center p-1 rounded-md items-center'>${ +listing.regularPrice - +listing.discountPrice }</p>
                    )
                }
            </div>
            ...
            <ul className='text-green-900 font-semibold text-sm flex 
            flex-wrap items-center gap-4 sm:gap-6'>
                ...
                <li className='flex items-center gap-1 whitespace-nowrap'>
                    ...
                    {listing.furnished ? 'Furnished spot' : 'Unfurnished'}

                </li>
            </ul>
            {currentUser && listing.userRef !== currentUser.data.userData._id && !contact && (
                <button onClick={() => setContact(true)} className='bg-slate-700 text-white rounded-lg
                uppercase hover:opacity-70 p-3'>Contact Landlord</button>
            )}
